...
provide this feature, but this drawback is balanced by the ability for
...
choosen such that they interact correctly with the state type.
...
\end{lstlisting}
% [KA]: maybe a for_each3 would be better, since it is exactly the Euler
% formula.
...
We describe odeint adaptation for GPGPU libraries under consideration in the
paragraphs below. We should mention that odeint now supports all of the
considered libraries natively. However, providing adaptation examples serves
our goal of comparing convenience of the libraries interfaces.

To adapt Thrust to odeint we need to provide both algebra and operations. The
algebra should define \code{for_each} family of algorithms for up to 8
arguments. All of these operations look very similar to each other, so we
only provide \code{for_each3} code here:
\begin{lstlisting}
...
device functor which uses \code{thrust::get<>} functions to unpack the zip
iterator into separate values. This is approach is heavily used with Thrust and
allows to process several vectors in single efficient sweep. Adaptation of
resizing operations is trivial and is omitted for the sake of conciseness.
...
\code{default_operations}. This combination proved to be convenient and
effective with VexCL where each expression results in a single kernel. For
ViennaCL though default operations involving more than two terms result in
multiple kernel launches.  Moreover, temporary vectors are allocated and
deallocated for each of such composite operations.  This results in dramatic
decrease of performance, so we had to provide specialized operations for
ViennaCL. These use custom kernel generation mechanism provided by the library.
For example, \code{scale_sum3} operation is defined as:
\begin{lstlisting}
...
The linear operator $\Delta_d q_{i,j} - \omega^2_{i,j}q_i$ may be represented
...
operator $\Delta_d q_{i,j} - \omega^2_{i,j}q_i$:
\begin{lstlisting}
typedef vex::vector<double> state_type;
