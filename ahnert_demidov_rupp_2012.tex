...
\begin{abstract}
    We present a comparison of several modern C++ libraries intended for
    OpenCL and CUDA programming. The comparison is based on odeint,
    a framework for the solution of ordinary differential
    ...
    allows to effectively use libraries such as VexCL, ViennaCL, or
...
Programmers needs to familiarize
...
example of using a range algebra. Here \code{for_each2} is used, which means that
...
ensemble example is presented on \figref{code:thrust:damped}. The functor holds model
parameters and provides \code{operator()} with a signature required by the odeint
library. The state type is represented by \code{thrust::device_vector<double>}.
...
zip iterator and passes them to \code{thrust::for_each} algorithm together with the 
provided device functor. There, individual components are unpacked again and the
operations required are applied to the derivative part. This technique allows
for the computation of the system function in single efficient pass resulting in a single CUDA
kernel launch.
...
\end{figure}
% [KR]: This example is too lengthy...


\figref{code:vexcl:damped} shows system functor for the VexCL version of the
damped harmonic oscillator example. The functor looks much simpler than the
Thrust variant because VexCL supports a rich set of vector expressions. The state
is represented by the \code{vex::multivector<double,2>} type which holds two
instances of \code{vex::vector<double>} and transparently dispatches all
operations to the underlying components. It would be in principle possible to
...
\eqref{eq:dampedsystem}. The drawback of this variant is that it would lead
to two kernel launches (one per each vector assignment) and would result in
...
% [KR]: It's probably better to flip the paragraph: First say that a direct representation in possible, then say how performance can be further improved. A comparison in the benchmarks would also be of interest.
...
The ViennaCL version of the damped harmonic oscillator example is presented on
\figref{code:viennacl:damped}. \code{boost::fusion::vector} is used to pack
coordinate components of the state into a single type. Individual components
are instances of \code{viennacl::vector<double>} type. We also had to adapt
odeint resizing mechanism to the resulting structure. Its implementation is
straightforward and omitted here for the sake of conciseness.
...
\end{figure}
%% [KR]: I suggest to split the code snippet into two parts: The kernel generation, and the execution phase.
...
\rem{As a second example we consider a chain of coupled phase oscillators. A
...
grows linearly in time $\de \phi / \de t = \omega$. $\omega$ is here
the phase velocity. Due to the absence of any information about the
amplitude of the oscillator, interesting behaviour can only be observed
if many of such oscillators are coupled. In fact, such a system can be
used to study divergent phenomena such as synchronization, wave and
...
\begin{equation} \label{eq:phasesystem}
    ...
    - \phi_{i-1}).
\end{equation}
Note that $\phi_i$ is a function of the time and the index $i$ denotes the
...
The VexCL version of the example is presented on \figref{code:vexcl:phase}.
Again, it is the most concise variant since VexCL provides native support for
the user-defined stencils operations. The sum of sines in
\eqref{eq:phasesystem} is encoded using the \code{vex::StencilOperator<>} class
template.  Template parameters are the body string for the generated OpenCL
function encoding required operation, width and center point of the stencil,
and its return type. Once the stencil operator is defined, the functor's
\code{operator()} is implemented with a single line of code.

\figref{code:viennacl:phase} presents the ViennaCL version of the phase oscillator
...
\end{figure}
% [KR]: Again, this rather lenghty code needs to be condensed and/or split.
...
The VexCL version is very simple:
\begin{lstlisting}[frame=leftline]
typedef vex::vector<double> state_type;
...
The user-defined OpenCL function \code{pow3} computes the third power of its argument and is
...
It may be said that the performance obtain so far is not optimal. Indeed, generic
odeint algorithms use temporary vector variables. For example, the fourth-order
...
It is of course possible to manually create computational kernels corresponding
to the full integration step. \figref{code:customkrn} shows one such
kernel. Unfortunately, by choosing this path we lost generality that odeint
provides.
% [KR]: The paragraph above needs improvements, it reads a bit bumpy...
...
VexCL provides a kernel generation mechanism that is able to solve this
problem. It allows for a transparent conversion of generic CPU algorithms
(restrictions apply) to OpenCL kernels. In order to do this conversion one
...
% [KR]: I'd keep that out of the paper. It undermines the first examples and opens up a completely new topic. I suggest to write a separate paper on that.
...
Thrust is more low-level and its interface is very close to the C++ STL library.
The OpenCL libraries we looked at demonstrated that they are able to 
provide more convenient interface for a scientific programmer than a direct implementation in CUDA or OpenCL.
VexCL has richer set of elementwise vector operations,
while ViennaCL has extensive set of sparse linear systems solvers,
which we did not discuss in this paper.

Regarding a comparison of CUDA versus OpenCL, we have noticed that OpenCL has two major
differences with respect to CUDA. First, it hasa  much wider range of supported
hardware, which is only going to extend with time because it is an open standard
supported by major hardware producers. The second advantage of OpenCL is at the
same time its drawback: one has to (or may to) compile his kernels at runtime  %[KR]: changed 'compiletime' to 'runtime'!
time. This adds certain overhead notable for smaller workloads, but at the same time
allows for building more efficient kernels transparently, as has been
shown in this paper. 
