...
\figref{code:vexcl:damped} shows system functor for VexCL variant of the damped
...
The ViennCL variant of the damped harmonic oscillator example is presented on
...
Thrust variant for the coupled phase oscillator chain looks very similar to the
...
VexCL variant of the example is presented on \figref{code:vexcl:phase}. Again,
...
\figref{code:viennacl:phase} presents ViennaCL variant of the phase oscillator
...
\begin{equation} \label{eq:latticesystem}
    \frac{dx}{dt} = -\Delta Kx + Dx - \beta x^3.
\end{equation}

Thrust does not provide any means for sparse matrix-vector multiplication, so
we had to omit the library from this experiment.
