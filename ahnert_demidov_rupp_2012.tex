...
% [KA]: The top 500 list is written as TOP500, in wikipedia an on the webpage.
...
% [DD] Sorry, I've never used anything but simple Euler method for the purpose
% of solving non-stationary PDEs. Usually implicit methods show better
% stability and require less time steps.


Odeint solves % [KR]: 'usually solves' is a bit too fuzzy. Does it support other types (implicit ODEs, higher-order ODEs)? If yes, we should state that here.  It is advisable to define the scope properly in a SIAM Journal. %[KA] implicit ODEs are differential algebraic equations, which can not be solved odeint, high order ODEs can always be transformed to be first-order. But odeint only solves the IVP.
...
Numerous methods for solving ODEs exist \cite{Press-92,HairerSolvingODEI,
  HairerSolvingODEII}, % [KR]: I would prefer a more diverse set of authors here (not just Hairer), % [KA] These are the standard books
...
\end{lstlisting}
The state type, the algebra, and the operations enter the Euler method  % [KR]: It is not yet defined what a 'stepper'... [KA]: right
...
\code{m_dxdt}. The call of \code{for_each3} iterates over all elements of three vectors in parallel and    % [KR]: This is a bit confusing. What does 'iterate three vectors' mean in this context? It is not obvious how the for_each3-scale_sum2-call equals x(t) + \Delta t \; f(x(t),t) . [KA]: ok I have change the formulation a bit
...
% [KA] I put the paragraph about algebra and reference example again into the text. I think it is ok to have them here, altough later we will show other implementations.
...
% [KA]: Hmm, I think the listing for the algebras is not needed, but I think
%       a implementation of for_each and scale_sum is interesting for the reader.

% [DD]: May be move the for_each and scale_sum implementations right after the
% sentence 'The call of \code{for_each3} iterates over all elements...' above
...
\end{equation}
with $\varepsilon = \varepsilon_0 + \varepsilon_a \cos \left( \omega_d    % [KR]: We should give \varepsilon a name here. Is it a 'damping function'? [KA]: it already has a name, see next sentence
...
ViennaCL supports expression templates for a limited set of    %[KR]: Denis, isn't VexCL supporting arbitrary vector operations? Are you referring with 'as well as VexCL' to expression templates only? 
...
% [DD]: Added definition for the state_type:
\begin{lstlisting}
...
% [KR]: Which SDK was used for the CPU computations? We should definitely
% consider a comparison of Intel vs. AMD SDKs. There could be quite a number of
% interesting differences.

% [DD]: I am running the experiments now, will add data when its ready.
...
\begin{figure}
    \begin{center}
        \includegraphics[width=\textwidth]{data/damped_oscillator/perfmtx}
    \end{center}
    \caption{Damped harmonic oscillator ensemble results.} 
    \label{fig:damped:perf}
\end{figure}
...
% [KR]: What about results for Tahiti? Either include results for all examples, or for none.
% [DD]: I'll add the rest of the Tahiti results.
...
\begin{figure}
    \begin{center}
        ...
        ]{\label{fig:scaling:damped}\includegraphics[width=0.32\textwidth]{data/damped_oscillator/scaling}}$\;$
        \subfigure[
