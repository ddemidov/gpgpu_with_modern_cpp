...
method one can imagine the is the explicit Euler scheme
\begin{equation}
...
applies on each element \code{scale_sum2}. The operation is here
...
the operations must be choosen such that they interact correctly with
the state type.
...
{
    // ...
    template< class R1 , class R2 Op op >
    void for_each2( R1 &r1 , R2 &r2 , Op op ) {
        auto it1 = boost::begin( r1 );
        auto it2 = boost::begin( r2 );
        for( ; it1 != boost::end( r1 ) ; )
            op( *it1++ , *it2++ );
    }
    // ...
};
\end{lstlisting}
The operations are a struct with public member classes defining the
operations which are used be the algebras. There is only one default
...
To adapt Thrust to odeint we need to provide both algebra and
...
\code{default_operations}. This combination proved to be convenient
and effective with VexCL where each expression results in a single
