...
ordinary differential equations (ODEs) numerically that has been recently been
...
value $x(t+\Delta t)$.  $\Delta t$ is the step size and many be chosen either
...
template parameter, hence they are exchangeable. The function object \code{ode}
...
$f(x(t), t)$.  Each of these algebras is a class consisting of
...
The ViennaCL version of the Lorenz attractor example uses ViennaCL's mechanism
for custom kernel generation that was described in
Sec.~\ref{sec:adapting-odeint}. The symbolic variables and the custom operation
...
included in the CUDA Toolkit distribution as well as in the Thrust library
...
the left show absolute time against the size of the problem being solved, while
...
differences between the libraries are more pronounced. Thrust outperforms VexCL
on larger problems by 15 to 40 percent, except for the phase oscillators
example, where VexCL is faster by about 5 percent.  The performance of ViennaCL
on the CPU is poor and about 75 percent below that of Thrust and between 30 and
70 percent below that of VexCL.  The only exception is again the phase
oscillator test, where ViennaCL outperforms other libraries. The difference
...
AMD's one is the chain of phase oscillators. The performance difference between
the implementations here reaches 30\%. % [DD] this needs some words.
...
\begin{table}
 \centering
    ...
    \begin{tabular}{|lc|rrr|}
        ...
        \hline
        \em Thrust   &\em Tesla & 242.78 & 263.99 & 319.54 \\
        \em VexCL    &\em Tesla & 246.58 & 189.38 & 401.31 \\
        \em ViennaCL &\em Tesla & 259.85 & 165.21 & 433.24 \\
        \hline
        \em VexCL    &\em Tahiti & 149.49 & 108.83 & 228.93 \\
        \em ViennaCL &\em Tahiti & 148.69 &  83.61 & 213.22 \\
        \hline
        \em Thrust   &\em CPU         & 2~336.14 & 5~172.84 &      N/A \\
        \em VexCL    &\em CPU (AMD)   & 3~203.47 & 5~015.02 & 3~152.92 \\
        \em VexCL    &\em CPU (Intel) & 3~284.18 & N/A & N/A \\
        \em ViennaCL &\em CPU (AMD)   & 4~073.66 & 4~605.93 & 5~485.99 \\
        \em ViennaCL &\em CPU (Intel) & 3~943.59 & N/A & N/A \\
        \hline
    \end{tabular}
\end{table}
