...
\begin{abstract}
    ...
    as MTL4, Thrust, VexCL, or ViennaCL, using CUDA or OpenCL technologies.
...
\begin{keywords}
    GPGPU, OpenCL, CUDA, Modern \Cpp libraries, odeint, Thrust, VexCL, ViennaCL, MTL4
\end{keywords}
...
or OpenCL frameworks.  The GPGPU libraries considered in this work are MTL4, Thrust,
VexCL, and ViennaCL:

% [DD] How do you like it this way? The complete odeint description is above
% and what follows is the list of the libraries we tested.
...
\begin{description}
    \item[MTL4] (The Matrix Template Library)\footnote{ \href{ http://mtl4.org }{
        ...
        Currently, three versions exist: the open-source editions that support
        single- and multi-core CPUs, the supercomputing edition providing generic
    MPI-based parallelism, and the CUDA edition that is introduced in this paper.
    ...
    \item[ViennaCL] (The Vienna Computing Library) is a scientific computing
        ...
        http://viennacl.sourceforge.net }} written in \Cpp and primarily based on OpenCL
...
\begin{sloppypar}
  ...
  combination proved to be effective with VexCL and MTL4, where each
...
more compact than the Thrust variant 
when  a rich set of
vector expressions
as in MTL4 or VexCL is available.
MTL4 provides the type \code{multi_vector} that allows for expressing the operations
directly:
\begin{lstlisting}
...
is dominant and one large vector operation is up to 150\,\% faster then three smaller
ones whereas the difference is less about 5\,\% when the vectors are larger than one
million entries.
...
to, or is used to increment/decrement the vector:
\begin{lstlisting}
y= S * x;
y+= S * x;
y-= S * x;
\end{lstlisting}
The user must provide a functor-like type that applies the stencil on 
the i-th element of a vector and its neighbors.
For the sake of performance the functor must provide two functions:
one that is checking indices and to be applied near the beginning and the end of the vector 
and one function without index checking.
For the considered example, the functor/kernel is:
\begin{lstlisting}
...
defined suitable for the according platform, e.g.~to an empty string on regular compilers.
Thus, the user code is entirely platform-independent.

The stencil functor is used as template argument of the stencil matrix:
\begin{lstlisting}
...
struct phase_oscillators
{
    ...
    sys_func(const State& omega) 
      : omega(omega), tmp(num_rows(omega), S(num_rows(omega)) {}
...
The need for defining a temporary vector explicitly in the user code 
instead of creating it implicitly in the expression template
 is not particularly elegant but unfortunately
needed for performance.
The allocation and deallocation of GPU memory in each function
call is extremely expensive (what can be clearly shown with a
GPU-supporting performance tracer~\cite{Juckeland12}).
For the future, we consider managing the memory of temporaries by hand to avoid 
such overhead.
...
%To simplify the programming, we wil
...
\end{lstlisting}
The expression \code{q * q * q} computes the triple element-wise product of 
column vector \code{q}.
Usually, products of column/row vectors among themselves are often program errors
and therefore not allowed in MTL4. 
It can be enabled by defining an according macro
during compilation.


In contrast to the other libraries, we used a compressed row storage matrix (CRS)
which is known for being a sub-optimal storage format on GPUs because it causes many non-coalesced 
memory accesses when the matrix is processed row-wise.
However, the corresponding matrix vector product can be realized with a special implementation when
all rows of the matrix have the same number of non-zero entries (or are accordingly filled with zeros). 
In this case, 
it is much easier to use shared memory for minimizing non-coalesced
global memory access.
This is subject to a future publication.
There is further potential in computing the matrix vector product and the element-wise
vector operations in one single loop as successfully demonstrated in MTL4's CPU implementation.
...
of an auxiliary index vector.  MTL4, VexCL and ViennaCL are in general slower than
...
Hamiltonian lattice experiment. 
Both OpenCL libraries show very similar results
on a GPU.
The specialized CRS implementation in MTL4 is twice as fast as  Thrust/CUSPARSE for very
small examples and up to 50\,\% slower for large examples---which is still significantly
below what Kraus and F\"orster observed that similar matrices are about four times faster
in ELLPACK format than CRS~\cite{KrausFoerster}.
...
%We believe that this may be explained by more effective implementation of the
%sine function in Intel OpenCL SDK.
% [DD] If I replace sine with erfc in system function, then Intel and AMD
% perform similarly.
% [DD] erfc does not help with viennacl (Intel is still much faster there), so..
% [DD] Karl, do you have any other ideas? Faster sines? better work with caches?
% [KR] I have no idea, probably it is indeed the sine() implementation. I've added that the test was run on an Intel CPU, maybe this causes the differences.
...
very close to the \Cpp STL library.  Special purpose functionality is available
