...
by NVIDIA tailored for general purpose computing on NVIDIA graphics adapters.
CUDA could be used theoretically for other platforms as well but tool support
only exists for NVIDIA hardware.
...
\begin{sloppypar}
  The three libraries MTL4, VexCL and ViennaCL provide convenient
...
In a similar manner, the system function is implemented in MTL4
using \code{multi_vector}:
\begin{lstlisting}
...
However, both techniques require a very subtle meta-programming treatment on the type level and
are not yet available CUDA version of MTL4 (we will refer to the CUDA version as CMTL4).
...
Its application is expressed by a matrix vector product that assigned
to a vector or used to increment/decrement it:
\begin{lstlisting}
y= S * x;
y+= S * x;
y-= S * x;
...
\end{lstlisting}
The stencil matrix ``S'' in the system function above
uses shared memory to benefit from re-accessing vector entries
and to avoid non-coalesced memory accesses.

The need for defining a temporary vector in the user code and referring
it in the system function is not particularly elegant but unfortunately
needed for performance.
Profiling of CUDA programs with trace tools like Vampir~\cite{vampir}
revealed that allocation and deallocation of GPU memory is extremely expensive
so that the creation of temporary variables often took more time 
than all calculations.
It is therefor avoided in the operator above.
On the other hand, odeint copies the system function object so that a member variable
would also cause expensive creation, destruction, and copy operations.
...
and therefor not allowed in MTL4. 
It can be enabled by defining the macro \code{MTL_WITH_SHORT_ELE_PROD} (e.g.~as compiler flag).

In contrast to the other libraries, we used a compressed row storage matrix (CRS).
The corresponding matrix vector product provides a special implementation when
all rows have the same number of non-zero entries. 
In this case, it is much easier to use shared memory for minimizing non-coalesced
