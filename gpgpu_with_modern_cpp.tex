...
which has been included into the Boost
...
arbitrary precision arithmetics, and can be easily adapted to use either CUDA
...
\begin{description}
    \item[MTL4] (The Matrix Template Library)\footnote{ \href{ http://mtl4.org }{
        http://mtl4.org }} is a \Cpp linear algebra library providing
        ...
        the open-source edition supporting single- and multi-core
        CPUs, the supercomputing edition providing generic MPI-based
        parallelism, and the CUDA edition introduced in this
...
sized problems. A portable pre-compilation to some low-level pseudo-code as in CUDA is
...
such a setting.  
%The implications of these two distinct design decisions in
%VexCL and ViennaCL will be further addressed in the discussion of the results. %[KR]: This is not further addressed actually...
...
considered in this work successfully use this approach and hide low-level details
from their users.
...
diverse fields as biology \cite{brauer_mathematical_2001,Murray-93},
...
\code{scale_sum2} to each triple. The operation is performed in-place,
...
\begin{sloppypar}
  ...
  which is able to generate specialized operations for ViennaCL. 
  %These
  %use a custom kernel generation mechanism provided by the library. %[KR] Sentence is redundant
...
struct lorenz_system {
    const vector_type &R;
    explicit lorenz_system(const vector_type &R) : R(R) { }
...
\end{lstlisting}
This class was originally designed for building a generic matrix as a set of  % [KR] Is this historical note important for the paper? Maybe rephrase?
column vectors and expressing all matrix operations in terms of operations 
...
The VexCL implementation of the Lorenz attractor ensemble example is as
compact as that of CMTL4. Here, the state is represented by the
...
launches, namely one per each vector assignment. As we have discussed previously
for the CMTL4 variant, this results in suboptimal
performance. A direct use of arithmetic operations for multi-vectors is
...
required implementation is only slightly longer than the above
variant:
\begin{lstlisting}[firstnumber=9]
    dxdt = std::tie(   sigma * (x(1) - x(0)),
                       ...
                       x(0) * x(1) - b * x(2)          );
\end{lstlisting}
The performance gain of these fused expressions is a bit larger
(25\,\% for large systems) compared to CMTL4. 
The reason might be the larger kernel launch overhead for OpenCL
kernels.
...
\code{viennacl::vector<double>} type.  The ViennaCL kernel generation facility already used in
...
The Thrust version for the coupled phase oscillator chain is very similar to
...
neighboring vector elements are needed. In order to access these values, we use Thrust's
...
vector elements. The function object
\code{phase_oscillators_functor} implements \eqref{eq:phasesystem} similarly to the \code{lorenz_functor} above and is thus omitted for brevity.
...
defined suitably for the according platform, e.g.~to an empty string on regular
compilers.  This makes the user code entirely platform-independent.
...
struct phase_oscillators {
    ...
    phase_oscillators(const State& w) : omega(w), S(num_rows(w)) {}
...
\code{VEX_STENCIL_OPERATOR} preprocessor macro. Its parameters are the name of the
resulting function object, the return type, the stencil width, the center, and the body string for
the generated OpenCL function encoding the required operation.  Once the stencil
...
\code{thrust::transform()} algorithm is used in line 5 to compute the
scaled third power of the input vector \code{q}. Lines 7--9 call the sparse
...
often program errors and therefore not allowed in CMTL4.  Their use may be
...
installed. OpenCL implementations from AMD and from Intel were used on the CPU and from AMD and NVIDIA on GPUs.  The
...
and AMD Tahiti boards are correspondingly plotted with solid and dotted lines.
...
the achieved memory bandwidth in GB/sec and in fractions of the theoretical peak
for each of the compute devices.
...
implementation of the phase oscillator chain example for Thrust is rather
ineffective since it uses a permutation iterator requiring an additional
global vector for storing indices. The implementations of stencil operations 
in CMTL4 and ViennaCL are slightly more efficient than those of VexCL.

Moreover, the overhead of using high-level libraries is negligible compared
...
Thrust, VexCL, and ViennaCL show very similar performance on the CPU for
larger problem sizes. For smaller problems the difference between Thrust
...
interface very close to the \Cpp Standard Template Library. Special purpose
