...
\end{lstlisting}
In this context, the class \code{multi_vector} is used in two ways:
expressing  operations on sub-vectors and on entire vectors.
Each operation on sub-vectors of  \code{x} and
\code{dxdt} causes a kernel call. 
...
required implementation is only slightly less intuitive than the above
variant:
\begin{lstlisting}[firstnumber=9]
