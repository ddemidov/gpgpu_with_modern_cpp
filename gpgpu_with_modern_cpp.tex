...
timings provided were obtained on Gentoo Linux operating system for two GPUs,
...
Thrust version with two exceptions. The first exception is
\figref{fig:phase:perf}, where ViennaCL is selected as a reference library for
GPU data due to poor performance of Thrust library in case of the coupled phase
oscillator chain example. The other exception is \figref{fig:lattice:perf},
where VexCL was selected as a reference library, since Thrust/CUSPARSE
combination is not able to run on a CPU. 
