close all
clear all

test = {'thrust_gpu', ...
	'vexcl_1gpu',   'vexcl_1gpu_multiops', ...
	'viennacl_gpu', 'viennacl_gpu_multiops'};

lgnd = {'Thrust GPU', ...
	'VexCL cust.kernel',    'VexCL multiops', ...
	'ViennaCL cust. kernel', 'ViennaCL multiops'};

style = {'k:', ...
	 'kd-', 'ko-', ...
	 'kd--', 'ko--'};

fcolor = {'k', ...
          'w', 'w', ...
	  'w', 'w'};

msize = 5;

figure(1)
set(gca, 'FontSize', 18);

idx = 0;
for t = test
    idx = idx + 1;
    data = load([cell2mat(t) '.dat']);
    avg = [];

    n = unique(data(:,1))';
    for i = n
	I = find(data(:,1) == i);
	time = median(data(I,2));
	avg = [avg time];
    end

    if idx == 1
	ref_avg = avg;
    end

    semilogx(n, avg ./ ref_avg, style{idx}, 'markersize', msize, 'markerfacecolor', fcolor{idx});
    hold on
end

xlim([1e2 1e7])
set(gca, 'xtick', [1e2 1e3 1e4 1e5 1e6 1e7])
xlabel('N');
ylabel('T / T(Thrust GPU)');
legend(lgnd, 'location', 'NorthEast');
legend boxoff
axis square

print('-depsc', 'multiops.eps');
